...
window.player = new (class {
  ...
  initEvents() {
    this.audioPlayer.ontimeupdate = () => {
      ...
      if (ps <= 100) {
        this.uiPlayer
          .find(".track .selectableTrack .slider")
          .attr("style", `left:${ps}%`);
    ...
    this.audioPlayer.onprogress = (e) => {
      ...
      if (buffered.length > 0) {
        ...
        if (this.uiPlayer.length > 0) {
          this.uiPlayer
            .find(".track .selectableTrackLoadProgress .load_bar")
            .attr("style", `width:${percentage.toFixed(2)}%`);
        }

        if (this.linkedInlinePlayer) {
          this.linkedInlinePlayer
            .find(".lengthTrackWrapper .selectableTrackLoadProgress .load_bar")
            .attr("style", `width:${percentage.toFixed(2)}%`);
        }
      }
    ...
    this.audioPlayer.onended = async (e) => {
      ...
      if (
        !this.nextTrack &&
        window.player.context.playedPages.indexOf(1) == -1
      ) {
      ...
      if (this.linkedInlinePlayer && this.connectionType) {
        ...
        const real_current = parent.find(
          `.audioEmbed[data-realid='${this.current_track_id}']`,
        );
  ...
  async loadContext(page = 1, after = true) {
    ...
    switch (this.context.object.name) {
      ...
      case "playlist_context":
        form_data.append("context", this.context.object.name);
        form_data.append("context_entity", this.context.object.entity_id);
        form_data.append("context_entity2", this.context.object.entity_id2);
        break;
  ...
  async setTrack(id, ref = null) {
    ...
    if (
      ...
      (!this.context.object ||
        this.context.object.url != location.pathname + location.search)
    ) {
    ...
    if (
      ...
      await this.loadContext(
        Number(Math.max(...this.context["playedPages"])) + 1,
        true,
      );
    }
    ...
    if (
      !this.previousTrack &&
      this.hasContext() &&
      Math.min(...this.context["playedPages"]) > 1
    ) {
      await this.loadContext(
        Math.min(...this.context["playedPages"]) - 1,
        false,
    ...
    if (this.isAtAudiosPage()) {
      document.title = ovk_proc_strtr(
        escapeHtml(
          `${window.player.currentTrack.performer} — ${window.player.currentTrack.name}`,
        ),
        255,
      );
    }
  ...
  async loadDump(dump_object) {
    ...
    if (
      dump_object.current_track_id &&
      this.hasTrackWithId(dump_object.current_track_id)
    ) {
  ...
  __highlightActiveTrack() {
    ...
    u(
      `.audiosContainer .audioEmbed[data-realid='${this.current_track_id}'] .audioEntry, .audios_padding .audioEmbed[data-realid='${this.current_track_id}'] .audioEntry`,
    ).addClass("nowPlaying");
  }
  ...
  __setFavicon(state = "playing") {
    if (state == "playing") {
      document
        ...
        .setAttribute(
          "href",
          "/assets/packages/static/openvk/img/favicons/favicon24_paused.png",
        );
    } else {
      document
        .querySelector('link[rel="icon"], link[rel="shortcut icon"]')
        .setAttribute(
          "href",
          "/assets/packages/static/openvk/img/favicons/favicon24_playing.png",
        );
    }
  }
  ...
  __updateFace() {
    ...
    if (!this.previousTrack) {
      ...
      prev_button.attr(
        "data-title",
        ovk_proc_strtr(escapeHtml(this.previousTrack.name), 50),
      );
      if (this.ajaxPlayer.length > 0) {
    ...
    if (!this.nextTrack) {
      ...
      next_button.attr(
        "data-title",
        ovk_proc_strtr(escapeHtml(this.nextTrack.name), 50),
      );
      if (this.ajaxPlayer.length > 0) {
    ...
    if (_c) {
      this.uiPlayer
        .find(".trackInfo .trackName span")
        .html(escapeHtml(_c.getTitle()));
      ...
      performers.forEach((performer) => {
        this.uiPlayer
          .find(".trackInfo .trackPerformers")
          .append(
      ...
      this.uiPlayer
        .find(".trackInfo .trackPerformers")
        .html(`<a>${tr("track_unknown")}</a>`);
    }

    if (u(".bigPlayer #album_info").length > 0) {
      if (_c && _c.hasPlaylist() && u(".playlistInfo").length == 0) {
        u(".bigPlayer").addClass("album_shown");
        u(".bigPlayer #album_info img").attr("src", _c.getPlaylistCover());
        u(".bigPlayer #album_info #album_embed_name").html(
          escapeHtml(ovk_proc_strtr(_c.getPlaylistName(), 60)),
        );
    ...
    if (this.ajaxPlayer.length > 0) {
      if (_c) {
        this.ajaxPlayer
          .find("#aj_player_track_title b")
          .html(escapeHtml(_c.getPerformer()));
        this.ajaxPlayer
          .find("#aj_player_track_title span")
          .html(escapeHtml(_c.getTitle()));
      }
    }
    ...
    if (this.ajaxPlayer_CheckIfBlockHasBeenShown()) {
      ...
      this.ajaxPlayer
        .find(`.aj_track[data-id='${window.player.current_track_id}']`)
        .addClass("selected");
    }
  }
  ...
  __updateTime(new_time) {
    ...
    if (this.timeType == 1) {
      this.uiPlayer
        .find(".trackInfo .elapsedTime")
        .html(fmtTime(this.currentTrack.length));
    } else {
      this.uiPlayer
        .find(".trackInfo .elapsedTime")
        .html(getRemainingTime(this.currentTrack.length, new_time));
    }
  }
  ...
  ajaxPlayer_ApplyQueueBlock() {
    if (u("#ajax_audio_player #aj_player_tracks").length > 0) {
      return;
    }
    if (u("#ajax_audio_player").length == 0) {
      return;
  ...
  ajCreate() {
    ...
    miniplayer_template.attr(
      "style",
      `left:${previous_time_x}px;top:${previous_time_y}px`,
    );
    ...
    $("#ajax_audio_player").draggable({
      ...
      containment: "window",
      cancel:
        "#aj_player_track .selectableTrack, #aj_player_volume .selectableTrack, #aj_player_buttons",
      stop: function (e) {
...
u(document).on("click", ".audioEntry .audioEntryWrapper .status", (e) => {
  ...
  if (
    !e.target.matches("a, .withLyrics, .playIcon") &&
    !wrapper.hasClass("processed")
  ) {
...
u(document).on("click", ".audioEntry .playerButton > .playIcon", async (e) => {
  ...
  if (!window.player.hasTrackWithId(id) && !window.player.isAtAudiosPage()) {
    ...
    !window.player.hasTrackWithId(id) &&
    window.player.isAtAudiosPage()
  ) {
    window.player.__renewContext();
    await window.player.loadContext(
      window.__current_page_audio_context.page ?? 1,
    );
    if (!isNaN(parseInt(location.hash.replace("#", "")))) {
      const adp = parseInt(location.hash.replace("#", ""));
...
u(document).on(
  "click",
  ".bigPlayer .playButton, #ajax_audio_player #aj_player_play_btn",
  async (e) => {
    if (window.player.audioPlayer.paused) {
      await window.player.play();
    } else {
...
u(document).on(
  "click",
  ".bigPlayer .backButton, #ajax_audio_player #aj_player_next",
  async (e) => {
    await window.player.playNextTrack();
  },
);
...
u(document).on(
  "click",
  ".bigPlayer .nextButton, #ajax_audio_player #aj_player_previous",
  async (e) => {
    await window.player.playPreviousTrack();
  },
);
...
u(document).on("click", ".bigPlayer .elapsedTime", (e) => {
  if (window.player.current_track_id == 0) return;
...
u(document).on(
  "click",
  ".bigPlayer .additionalButtons .repeatButton, #ajax_audio_player #aj_player_repeat",
  (e) => {
    if (window.player.current_track_id == 0) return;
    ...
    if (targ.hasClass("pressed")) window.player.audioPlayer.loop = true;
    else window.player.audioPlayer.loop = false;
  },
);
...
u(document).on("keydown", (e) => {
  ...
  if ([32, 37, 39, 107, 109].includes(e.keyCode)) {
    if (window.messagebox_stack.length > 0) return;
  ...
  switch (e.keyCode) {
    ...
    case 37:
      ...
      window.player.audioPlayer.currentTime =
        window.player.audioPlayer.currentTime - arrow_click_offset;
      break;
    case 39:
      window.player.listen_coef -= 0.5;
      window.player.audioPlayer.currentTime =
      ...
      break;
    case 107:
      window.player.audioPlayer.volume = Math.max(
        Math.min(window.player.audioPlayer.volume + volume_offset, 1),
        0,
      );
      break;
    case 109:
...
u(document).on("keyup", (e) => {
  ...
  switch (e.keyCode) {
    ...
    case 82:
      document
        .querySelector(".bigPlayer .additionalButtons .repeatButton")
        .click();
      break;
    case 77:
      document
        .querySelector(".bigPlayer .additionalButtons .deviceButton")
        .click();
      break;
  }
});
...
u(document).on(
  "mousemove click mouseup",
  ".bigPlayer .trackPanel .selectableTrack, .audioEntry .subTracks .lengthTrackWrapper .selectableTrack, #aj_player_track_length .selectableTrack",
  (e) => {
    ...
    const time = Math.ceil(
      (width * window.player.currentTrack.length) / (rect.right - rect.left),
    );
    if (e.type == "mousemove") {
      let buttonsPresseed = _bsdnUnwrapBitMask(e.buttons);
      if (buttonsPresseed[0]) __defaultAction(time);
    }
    ...
    parent
      .find(".tip_result")
      .html(fmtTime(time))
      .attr("style", `left:min(${width - 15}px, 315.5px)`);
  },
);
...
u(document).on(
  "mousemove click mouseup",
  ".bigPlayer .volumePanelTrack .selectableTrack, .audioEntry .subTracks .volumeTrack .selectableTrack, #aj_player_volume .selectableTrack",
  (e) => {
    if (window.player.isAtAudiosPage() && window.player.current_track_id == 0)
      return;
    ...
    if (e.type == "mousemove") {
      let buttonsPresseed = _bsdnUnwrapBitMask(e.buttons);
      if (buttonsPresseed[0]) __defaultAction(volume);
    }
    ...
    parent
      .find(".tip_result")
      .html((volume * 100).toFixed(0) + "%")
      .attr("style", `left:${width - 15}px`);
  },
);
...
u(document).on(
  "contextmenu",
  ".bigPlayer, .audioEmbed, #ajax_audio_player",
  (e) => {
    if (e.shiftKey) {
      return;
    }
    ...
    u("#ctx_menu").remove();
    const ctx_type =
      u(e.target).closest(".bigPlayer, #ajax_audio_player").length > 0
        ? "main_player"
        : "mini_player";
    ...
    ctx_u.find("#audio_ctx_copy").on("click", async (e) => {
      if (ctx_type == "main_player") {
        if (window.player.current_track_id == 0) {
          makeError(
            tr("copy_link_to_audio_error_not_selected_track"),
            "Red",
            4000,
            80,
          );
        ...
        const url =
          location.origin +
          `/audio${window.openvk.current_id}_${window.player.current_track_id}`;
        await copyToClipboard(url);
      } else {
        const url =
          location.origin +
          `/audio${window.openvk.current_id}_${u(e.target).closest(".audioEmbed").attr("data-realid")}`;
        await copyToClipboard(url);
      }
    });
    ...
    ctx_u.find("#audio_ctx_mute").on("click", async () => {
      ...
      window.player.audioPlayer.muted = window.player.uiPlayer
        .find(".deviceButton")
        .hasClass("pressed");
    });
    ctx_u.find("#audio_ctx_add_to_group").on("click", async () => {
      if (ctx_type == "main_player") {
        ...
        __showAudioAddDialog(
          Number(u(e.target).closest(".audioEmbed").attr("data-realid")),
        );
      }
    });
    ...
    ctx_u.find("#audio_ctx_play_next").on("click", (ev) => {
      ...
      const move_id = Number(
        u(e.target).closest(".audioEmbed").attr("data-realid"),
      );
      if (current_id == 0) {
        return;
      }
      ...
      const next_track_player = u(
        `.audioEmbed[data-realid='${window.player.nextTrack.id}']`,
      );
      ...
      if (next_track_player.length > 0 && moving_track_player.length > 0) {
        next_track_player.nodes[0].outerHTML =
          moving_track_player.nodes[0].outerHTML +
          next_track_player.nodes[0].outerHTML;
...
u(document).on("click", ".musicIcon.edit-icon", (e) => {
  ...
  MessageBox(
    ...
    [
      function () {
        ...
        const t_explicit = document.querySelector(
          ".ovk-diag-body input[name=explicit]",
        ).checked;
        const t_unlisted = document.querySelector(
          ".ovk-diag-body input[name=searchable]",
        ).checked;
        ...
        $.ajax({
          ...
          success: (response) => {
            if (response.success) {
              ...
              perf.setAttribute(
                "href",
                "/search?q=&section=audios&order=listens&only_performers=on&q=" +
                  response.new_info.performer,
              );

              e.target.setAttribute(
                "data-performer",
                escapeHtml(response.new_info.performer),
              );
              e.target.setAttribute(
                "data-title",
                escapeHtml(response.new_info.name),
              );
              e.target.setAttribute(
                "data-lyrics",
                escapeHtml(response.new_info.lyrics_unformatted),
              );
              e.target.setAttribute(
                "data-explicit",
                Number(response.new_info.explicit),
              );
              e.target.setAttribute(
                "data-searchable",
                Number(!response.new_info.unlisted),
              );
              ...
              if (response.new_info.lyrics_unformatted != "") {
                if (player.querySelector(".lyrics") != null) {
                  player.querySelector(".lyrics").innerHTML =
              ...
              if (Number(response.new_info.explicit) == 1) {
                if (!player.querySelector(".mediaInfo .explicitMark"))
                  player.querySelector(".mediaInfo").insertAdjacentHTML(
                    "beforeend",
                    `
              ...
              if (url.searchParams.p != null) page = String(url.searchParams.p);
            } else fastError(response.flash.message);
          },
        });
      },
  ...
  window.openvk.audio_genres.forEach((elGenre) => {
    document
      .querySelector(".ovk-diag-body select[name=genre]")
      .insertAdjacentHTML(
        "beforeend",
        `
  ...
  const album_select = document.querySelector(
    ".ovk-diag-body select[name=album_id]",
  );
  (async () => {
    const res = await window.OVKAPI.call("audio.getAlbums", {
      count: 100,
      owner_id: owner_id,
    });
    res.items.forEach((album) => {
      album_select.insertAdjacentHTML(
        "beforeend",
        `
        ...
        () => {
          $.ajax({
            ...
            success: (response) => {
              ...
              if (response.success) u(player).remove();
              else fastError(response.flash.message);
            },
          });
        },
    ...
    hooks: {
      ...
      afterResponse: [
        async (_request, _options, response) => {
          ...
          if (json.success) $(e.currentTarget.closest(".audioEmbed")).remove();
          else fastError(json.flash.message);
        },
      ],
    },
    ...
    [
      ...
      () => {
        ...
        u("#_content .entity_vertical_list_item").nodes.forEach((item) => {
          const _checkbox = item.querySelector(
        ...
        switch (current_tab) {
          case "club":
            $.ajax({
              ...
              success: (response) => {
                if (!response.success) fastError(response.flash.message);
                else NewNotification(tr("audio_was_successfully_added"), "");
              },
            });
    ...
    switch (tab) {
        ...
        if (window.openvk.writeableClubs == null) {
          ...
          try {
            window.openvk.writeableClubs = await window.OVKAPI.call(
              "groups.get",
              { filter: "admin", count: 100, fields: "photo_50" },
            );
          } catch (e) {
      ...
      case "playlist":
        ...
        async function recievePlaylists(s_page) {
          res = await fetch(
            `/method/audio.searchAlbums?auth_mechanism=roaming&query=&limit=10&offset=${s_page * per_page}&from_me=1`,
          );
          res = await res.json();
        ...
        function appendPlaylists(response) {
          ...
          if (response.count > per_page * page) {
            u("#_content .entity_vertical_list").append(
              `<a id='_pladdwinshowmore'>${tr("show_more")}</a>`,
            );
          }
        }
        ...
        if (window.openvk.writeablePlaylists == null) {
          ...
          try {
            ...
            if (
              !window.openvk.writeablePlaylists ||
              window.openvk.writeablePlaylists.count < 1
            ) {
              throw new Error();
            }
          } catch (e) {
...
function showAudioAttachment(type = "form", form = null) {
  ...
  msg
    .getNode()
    .find(".ovk-diag-body")
    .attr("style", "padding:0px;height:335px");
  ...
  searcher.successCallback = (response, thisc) => {
    ...
    if (count < 1) {
      document.querySelector(".audiosInsert").innerHTML =
        thisc.context_type == "entity_audios"
          ? tr("no_audios_thisuser")
          : tr("no_results");
      return;
    }
    ...
    result.querySelectorAll(".audioEmbed").forEach((el) => {
      ...
      if (type == "form") {
        is_attached =
          u(form).find(
            `.post-vertical .vertical-attachment[data-type='audio'][data-id='${id}']`,
          ).length > 0;
      } else {
        is_attached =
          u(form).find(`.PE_audios .vertical-attachment[data-id='${id}']`)
            .length > 0;
      }
  ...
  u(".searchBox input").on("change", async (e) => {
    if (
      e.currentTarget.value === document.querySelector(".searchBox input").value
    ) {
      searcher.clearContainer();
  ...
  u(".audiosInsert").on("click", ".attachAudio", (ev) => {
    ...
    if (type == "form") {
      is_attached =
        u(form).find(`.post-vertical .vertical-attachment[data-id='${id}']`)
          .length > 0;
    } else {
      is_attached =
        u(form).find(`.PE_audios .vertical-attachment[data-id='${id}']`)
    ...
    if (is_attached) {
      if (type == "form") {
        u(form)
          .find(`.post-vertical .vertical-attachment[data-id='${id}']`)
          .remove();
      } else {
        u(form)
      ...
      if (
        type == "form" &&
        u(form).find(`.upload-item`).length > window.openvk.max_attachments
      ) {
  ...
  MessageBox(
    ...
    [
      function () {
        ...
        xhr.open(
          "GET",
          "/report/" + e.target.dataset.id + "?reason=" + res + "&type=audio",
          true,
        );
        xhr.onload = function () {
...
$(document).on("click", ".addToPlaylist", (e) => {
  ...
  if (!audios.value.includes(id + ",")) {
    ...
    e.currentTarget.querySelector("span").innerHTML = tr(
      "remove_from_playlist",
    );
  } else {
    document.querySelector("input[name='audios']").value = document
      .querySelector("input[name='audios']")
      .value.replace(id + ",", "");
    ...
    success: (response) => {
      if (response.success) {
        e.currentTarget.setAttribute(
          "id",
          id == "unbookmarkPlaylist"
            ? "bookmarkPlaylist"
            : "unbookmarkPlaylist",
        );
        e.currentTarget.innerHTML =
          id == "unbookmarkPlaylist" ? tr("bookmark") : tr("unbookmark");
...
u(document).on(
  "click",
  ".upload_container_element #small_remove_button",
  (e) => {
...
u(document).on("drop", ".PE_audios .vertical-attachment", (e) => {
  ...
  if (
    ...
    if (
      (!current.closest(".vertical-attachment").length < 1 &&
        target.closest(".vertical-attachment").length < 1) ||
      (current.closest(".vertical-attachment").length < 1 &&
        !target.closest(".vertical-attachment").length < 1)
    ) {
      return;
...
u(document).on(
  "click",
  ".PE_end #playlist_create, .PE_end #playlist_edit",
  async (e) => {
    const ids = [];
